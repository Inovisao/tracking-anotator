...
class AnnotationTool:
    ...
    def run_model(self, original_frame: np.ndarray) -> List[Detection]:
        """Executa o modelo YOLO (retificado ou original) e filtra detecoes."""
        detections: List[Detection] = []
        if (
            self.homography_matrix is None
            or self.warp_size is None
            or self.inverse_homography is None
            or original_frame is None
        ):
            return detections

        rectified = self.current_rectified_frame
        if rectified is None:
            rectified = self.warp_frame(original_frame)

        tries = []
        if USE_RECTIFIED_FOR_DETECTION:
            tries.append(("rectified", rectified, self.inverse_homography, self.warp_size))
        tries.append(("original", original_frame, None, original_frame.shape[1::-1]))

        for mode, frame_for_model, box_matrix, size in tries:
            if mode == "original" and not (FALLBACK_TO_ORIGINAL_IF_EMPTY or not USE_RECTIFIED_FOR_DETECTION):
                continue

            results = self.model.track(frame_for_model, persist=True, verbose=False)
            if not results or not hasattr(results[0], "boxes") or results[0].boxes is None:
                if mode == "rectified" and FALLBACK_TO_ORIGINAL_IF_EMPTY:
                    continue
                return detections
            ...
            for box in result.boxes:
                ...
                xyxy[0::2] = np.clip(xyxy[0::2], 0, frame_width - 1)
                xyxy[1::2] = np.clip(xyxy[1::2], 0, frame_height - 1)
                ...
                if tracker_id is not None:
                    if tracker_id not in self.tracker_id_map:
                        self.tracker_id_map[tracker_id] = self.allocate_track_id()
                    track_id = self.tracker_id_map[tracker_id]
                else:
                    track_id = self.allocate_track_id()
                self.register_track_id(track_id)

                if mode == "rectified":
                    original_box = self.project_bbox(xyxy, self.inverse_homography, img_width, img_height)
                    warp_box = xyxy
                else:
                    original_box = clip_bbox(xyxy[0], xyxy[1], xyxy[2], xyxy[3], img_width, img_height)
                    warp_box = None
            ...
            if detections or not FALLBACK_TO_ORIGINAL_IF_EMPTY:
                break
        return detections
